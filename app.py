...
def calcular_metricas_partido(item, nombre_liga):
    ...
    var_local = 0.85 + ((hash_base % 100) / 100.0) * 0.50
    var_visita = 0.70 + (((hash_base // 10) % 100) / 100.0) * 0.50
...
def aplicar_colores(val):
    if isinstance(val, (int, float)):
        if val >= 80:
            return 'background-color: #1e4620; color: #75fb8d; font-weight: bold;'  # Verde destacado (≥ 80%)
        elif val >= 75:
...
if 'df_partidos' in st.session_state and st.session_state.get('fecha_cargada') == fecha_consulta:
    ...
    with f_col2:
        # Menú configurado con rango desde 75% en adelante
    ...
    # Filtros desde el 75% en adelante
